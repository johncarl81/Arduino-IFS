...
void lcdInit() {
 ...
 // Color set
  ...
  for(int i = 0; i < 500; i++){
    ...
    double x1 = ((affine[a][0] * x) + (affine[a][1] * y) + affine[a][4]);
    double y1 = ((affine[a][2] * x) + (affine[a][3] * y) + affine[a][5]);
...
int randomAffine(){
  ...
  for(int i = 0; i < affineSize; i++){
    ...
    double affineProbability = affine[i][6] * 100;
